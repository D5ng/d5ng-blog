...
RHF를 분석하면서 callback ref를 사용하는 방식을 처음 접했다. 찾아보니, callback ref는 요소가 마운트되었을 때 바로 실행되어 useEffect 같은 추가적인 훅 없이도 초기화 작업을 수행할 수 있다고 한다. 또한, RHF에서는 기본적으로 에러가 발생한 필드에 포커스를 자동으로 맞추지 않지만, 에러 발생 시 해당 필드로 포커스가 이동되도록 구현하여 UX를 개선했다. (더 찾아봐보고 RHF에 이슈를 남기면 좋을것 같다는 생각도 든다)
