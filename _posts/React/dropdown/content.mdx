...
위와 같은 방식으로 만드는 경우가 생각보다 흔하며, 훅으로 로직을 분리하지 않는 경우도 많다. 처음에는 이 방식이 빠르지만 시간이 지남에 따라 개발 속도가 현저히 떨어진고 클라이언트 리소스 또한 많이 소모된다.
...
여기서 오해해서는 안 되는 부분이 있다. 위 코드는 컴포넌트의 형태를 UI 라이브러리와 비슷하게 만든 것이지, Radix UI처럼 만든 것은 아니다. 개선한 코드를 보면 `<DropdownMenuTrigger />` 컴포넌트 내부에 버튼이 포함되어 있다. 버튼은 중첩될 수 없기 때문에, button 태그가 들어있는 컴포넌트를 사용할 수 없는 문제가 발생한다. Radix UI는 **Render Delegation**을 사용해 이 문제를 해결하고 있다. 따라서 위 코드는 Radix UI와 유사하지만, 동일하다고 할 수는 없다.
