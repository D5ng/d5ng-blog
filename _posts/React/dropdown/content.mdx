...
드롭다운 메뉴는 웹 애플리케이션에서 흔히 볼 수 있는 UI다. 누구나 한 번쯤 만들어 본 경험이 있을 것이다. 만드는 것도 크게 어렵지 않다. 내가 부트캠프를 하면서 많은 사람들의 드롭다운 메뉴는 재사용성과 확장성이 떨어져, 남의 코드에 if문이 덕지덕지 붙거나 다시 만든 경험이 있었다. 프론트엔드를 배운 대부분의 사람은 이를 구현할 수 있다. 하지만 구현에만 초점을 둔 것과 확장성과 유연성을 고려한 것과는 시간이 지남에 따라 큰 차이가 나타난다. 따라서, 구현에만 초점을 둔 초안을 바탕으로 점진적으로 어떻게 개선하는지에 대한 나의 방법을 공유하겠다.
...
즉, 서로 비슷한 UI이지만 기능에 초점이 다르기 때문에 요구사항 또한 다르다. 실제로 나도 드롭다운 메뉴와 셀렉트를 동시에 사용하려다 보니 코드가 많이 지저분해졌던 경험이 있었다.
...
취업 준비 중인 신입 프론트엔드 개발자들은 보통 다음과 같은 형태로 만들곤 한다. 구글에서 검색해보면 비슷한 방식으로 만든 포스트들이 많이 있다.
...
무의식적으로 쉽게 만드는 UI 컴포넌트는 정말 많다. 그러나 본인이 만든 컴포넌트가 좋은지에 대해 의심해볼 필요가 있다. 라이브러리에서 네이밍을 어떤 식으로 짓는지 확인도 해보고 어떤 방식으로 구현이 되었는지를 분석하면 컴포넌트를 설계함에 있어 많은 도움이 된다.

이 글에서는 Render Delegation까지 적용해 보여드리고 싶었지만, 라이브러리의 소스코드를 그대로 사용하면 되기에 생략하겠다. 관심 있는 분들은 아래의 링크를 참고하면 좋을 것이다.
