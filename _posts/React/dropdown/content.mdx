...
무의식적으로 쉽게 만드는 UI 컴포넌트는 정말 많다. 그러나 본인이 만든 컴포넌트가 충분히 좋은지 의심해보는 것이 중요하다. 이 글에서는 Render Delegation까지 적용해 보여드리고 싶었지만, 라이브러리의 소스코드를 참고하면 되므로 생략하겠다. 관심 있는 분들은 아래의 참조 링크를 참고하면 좋을 것이다.
