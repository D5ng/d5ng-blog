...
변수는 하나의 값을 저장하기 위해 확보한 메모리공간을 식별하기위해 붙인 이름을 말한다. 자세하게 풀어서 설명하자면 하나의 값을 저장하기 위해 확보한 메모리공간에, 메모리 주소가 있는데, 이 메모리 주소는 2진수로 되어있어 사람이 쉽게 알아볼수가없다. 그래서 변수이름으로 메모리 주소를 참조하게 되고, 메모리주소를 참조하게 된다는건 해당되는 메모리에 값을 가져올 수 있다.
